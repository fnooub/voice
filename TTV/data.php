...
foreach (json_decode($story['content']) as $url => $title) {
	$count++;
	$list[] = '<div><a href="'.base_url().'/bookmark.php?title=[TTV] '.$story['name'].' ➧ '.$title.'"><font color="black">['.$count.']</font></a> <a href="'.base_url().'/TTV/getTTV.php?link='.$url.'&slug='.$story['name'].'">'.$title.'</a></div>';
}

...
(function ($, window, document) {
    ...
    var chapStart = <?php echo $site['start']-1 ?>;
...
<?php if ($regex['flag'] == 'g'): ?>
		str = str.replace(/<?php echo $regex['s'] ?>/gm, '<?php echo $regex['r'] ?>');
<?php else: ?>
<?php if ($regex['flag'] == 'i'): ?>
		str = str.replace(/<?php echo $regex['s'] ?>/gmi, '<?php echo $regex['r'] ?>');
<?php endif ?>
<?php endif ?>
<?php endforeach ?>
...
<?php foreach ($regexs as $regex): ?>
<?php if ($regex['flag'] == 'td'): ?>
		str = str.replace(/<?php echo $regex['s'] ?>/, '<?php echo $regex['r'] ?>');
<?php endif ?>
<?php endforeach ?>
    	return str;
    }
    ...
    function getContent() {
        ...
        $.ajax({
            ...
            chapTitle = cleanTitle($data.find('#js-truyencv-read-content .title').text().trim());
...
<?php
file_put_contents("tcv.user.js", ob_get_contents());
header('location: tcv.user.js');
?>
